...
\paragraph{Directional certainty across the observed period.}
\label{subsec:method-certainty-contrasts}
We compare blaming and defending dissent separately in low- and
high-agreement posts using all retained annotated comments observed after
submission, without an upper elapsed-time cutoff. This is the full observed
period in the archive, not necessarily a post's complete lifetime; follow-up
coverage may differ across posts. Within each post, we calculate the mean minority certainty and
the proportion of minority comments assigned certainty level $4$, retaining
only YA/NA comments with valid scores in $\{1,2,3,4\}$. We then average these
post-level quantities with equal weight per contributing post. Posts without
a valid minority certainty observation in this period do not contribute;
their certainty is not set to zero. The two dissent directions correspond
to different post cohorts, not paired observations within the same post.

For each community and agreement band, we report blaming minus defending
differences in mean certainty and in the certainty-$4$ proportion. We obtain
percentile $95\%$ confidence intervals from $10{,}000$ bootstrap replicates,
resampling whole posts independently within each directional cohort. The
same sampled-post weights are used across measures in a replicate, and
intervals for differences are calculated from the replicate-level contrasts.
These are marginal intervals for descriptive sample contrasts, without a
joint multiplicity adjustment or population reweighting. Comparing whether
two intervals individually exclude zero is not a test of a
direction-by-agreement interaction. The dataset combines annotation versions
\texttt{v4} and \texttt{v4.1}; we additionally examine version-restricted
estimates as a sensitivity analysis.

We also repeat the contrasts using comments arriving in $[0,18)$ hours after
submission in both communities. The 18-hour reference for AITA's verdict
process motivates this sensitivity window; the same cutoff in AITAH provides
a comparable early-period summary, not an equivalent institutional event.
We additionally compare the full-period and first-18-hour estimates on the
same posts, restricted to those contributing a valid minority certainty
observation before 18 hours. For these within-cohort window comparisons,
each bootstrap replicate uses the same post weights for both windows.
Final-verdict labels and agreement bands remain fixed across windows.
This analysis assesses sensitivity to the time cutoff, not a causal effect
of the verdict rule (Appendix~\ref{sec:appendix-certainty-window}).
...
sampling design. The contributing counts for each trajectory window must be
reported for the high-agreement subset of the $1{,}195$-post dataset.
